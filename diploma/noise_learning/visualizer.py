...
class Visualizer:
    ...
    def set_metrics(self):
        agent_results = self.results_manager.get_results()
    ...
    def show_metrics(self):
        self.__plot_all_by_noise()
        # self.__plot_agent()
        plt.show(block=False)

    def __plot_all_by_noise(self):
        self.__plot_scores()
        self.__plot_losses()

    # def __plot_agent(self):
    #     fig = plt.figure()
    #     legend = []
    ...
    def __plot_scores(self):
        ...
        for i in range(self.agents_number):
            all_metrics.extend(self.agent_metrics[i].scores)
        ...
        for noise in all_metrics.get_unique_sorted_noises():
            ...
            plt.plot(avgs.get_metric_property("iteration"), avgs.get_metric_property("value"))
            legend.append(f"Noise = {noise:.2f}")
            
        fig.suptitle(f"Averaged Score for {self.results_number} run(s) per Noise")
    ...
    def __plot_losses(self):
        ...
        all_metrics: Metrics = Metrics()
        for i in range(self.agents_number):
            all_metrics.extend(self.agent_metrics[i].losses)
        ...
        fig.suptitle(f"Averaged Loss for {self.results_number} run(s) per Noise")
        plt.ylabel(f"Moving avg over the last {self.metrics_number_of_elements} elements every {self.metrics_number_of_iterations} iterations")
        plt.xlabel(f"Env Iterations")
        plt.legend(legend, loc='upper left')
